...
const Problems = memo(() => {
  const problemList = useSelector<Store.reducer, ASelector>(
    (state) => state?.problemsReducer?.payload
  );
  const [items, setItems] = useState<JSX.Element[]>();

  const render = useCallback((problemList: T.jsonData[]) => {
    return problemList.map((p, i) => {
  ...
  return (
    <Div className="problems-container" css={P.problemsContainer}>
      <Div className="header" css={P.header}>
        <Span className="title" css={P.title}>
          학습지 상세 편집
        </Span>
      </Div>
